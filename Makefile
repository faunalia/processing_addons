...
package: clean all
	cd .. && rm -f *.zip && zip -r processing_ogrgeoprocessing.zip processing_ogrgeoprocessing -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\* \*Makefile*
	mv ../processing_ogrgeoprocessing.zip .

upload: package
	plugin_uploader.py processing_ogrgeoprocessing.zip
